...
Beacon blocks can include a number of deposit receipts up to `MAX_DEPOSITS_RECEIPTS_PER_PAYLOAD`. Note that MAX*DEPOSIT_RECEIPTS_PER_PAYLOAD is an \_implied_limit on deposit operations and is equal to the number of deposits that can be packed into an execution block, given the gas limit (30 million gas). In comparison, the current `MAX_DEPOSITS_PER_BLOCK = 16 `constant that currently limits deposit operations is \_explicit*: a validator is expected to confirm that a proposed block doesn't include more than 16 deposits during Beacon block processing.
