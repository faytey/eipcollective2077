...
- Each leaf node (at every level of the tree) includes a succinct proof *π_of its membership in the vector committed to by its parent node. The membership proof *π*is derived with respect to the (vector) commitment \_C*—allowing a prover \_P_to convince a verifier \_V_that the value of a leaf is equal to the opening of the vector commitment \_C_at that specific position in the tree.
